...
## Overview
This document defines the standards for CLI command consistency across the MyBitcoinFuture CLI interface.

## Command Structure

### Primary Commands
- Use **kebab-case** for all primary commands
- Keep commands short but descriptive
- Use consistent verb-noun patterns

**Good Examples:**
\`\`\`bash
node cli/cli.js portfolio
node cli/cli.js list-wallets
node cli/cli.js add-wallet
node cli/cli.js remove-wallet
node cli/cli.js show-transactions
node cli/cli.js create-report
\`\`\`

**Bad Examples:**
\`\`\`bash
node cli/cli.js Portfolio
node cli/cli.js listWallets
node cli/cli.js add_wallet
node cli/cli.js transactions-global
\`\`\`

### Command Categories
- **System:** \`show-status\`, \`show-features\`, \`show-logs\`
- **Wallets:** \`list-wallets\`, \`add-wallet\`, \`remove-wallet\`, \`balance\`
- **Transactions:** \`show-transactions\`, \`show-transactions-global\`, \`show-transactions-summary\`
- **Users:** \`list-users\`
- **Alerts:** \`list-alerts\`
- **Reports:** \`list-reports\`, \`create-report\`
...
### Global Options
- Use consistent naming across all commands
...
- Follow consistent patterns for similar options
- Use kebab-case for multi-word options

**Good Examples:**
\`\`\`bash
--wallet-id <id>      # Wallet identifier
--transaction-id <id> # Transaction identifier
--start-date <date>   # Start date for filtering
--end-date <date>     # End date for filtering
--limit <number>      # Limit number of results
--offset <number>     # Offset for pagination
--xpub <xpub>         # Extended public key
--label <label>       # Wallet or item label
\`\`\`
...
- Use consistent formatting for numbers and dates

**Standard Table Format:**
\`\`\`bash
# Wallet List
ID    Label           Balance BTC    Balance USD    Status
1     Main Wallet     0.12345678     \$4,567.89     Active
2     Savings         0.05000000     \$1,850.00     Active

# Transaction List
TXID                    Amount BTC    Confirmations    Date
abc123...def456        0.00100000    6               2024-01-15 10:30:00
\`\`\`
...
- Use standard CSV formatting

**Standard CSV Format:**
\`\`\`csv
ID,Label,Balance BTC,Balance USD,Status
1,Main Wallet,0.12345678,4567.89,Active
2,Savings,0.05000000,1850.00,Active
\`\`\`
...
\`\`\`javascript
// Validation errors
ErrorHandler.ValidationError('Invalid xpub format')

// Not found errors
ErrorHandler.NotFoundError('Wallet not found')

// Search errors
ErrorHandler.SearchError('Search operation failed')

// Notification errors
ErrorHandler.NotificationError('Failed to send notification')

// Export errors
ErrorHandler.ExportError('Export operation failed')

// Import errors
ErrorHandler.ImportError('Import operation failed')
\`\`\`
...
\`\`\`bash
Error [E_INVALID_XPUB]: Invalid xpub. Please provide a valid xpub.
Error [E_WALLET_NOT_FOUND]: Wallet not found. Please check the wallet ID and try again.
Error [E_API_ERROR]: API request failed. Please check your connection and try again.
\`\`\`

### Error Recovery
- Provide clear recovery instructions
- Suggest alternative commands when appropriate
- Include help text for complex errors
...
function validateXpub(xpub) {
  return typeof xpub === "string" && xpub.startsWith("xpub") && xpub.length > 10;
}

// ID validation
function validateId(id) {
  return typeof id === "string" && id.match(/^\\d+\$/);
}
...
\`\`\`bash
node cli/cli.js add-wallet --xpub <xpub> --label <label>  # Both required
node cli/cli.js remove-wallet <id>                        # ID required
node cli/cli.js show-transactions --wallet-id <id>        # Wallet ID required
\`\`\`
...
**Help Text Format:**
\`\`\`bash
Usage: node cli/cli.js add-wallet [options]

Add a new Bitcoin wallet to the system

Options:
  --xpub <xpub>     Extended public key (required)
  --label <label>   Wallet label (required)
  --description <text>  Wallet description
  --network <network>   Network (mainnet|testnet) [default: mainnet]

Examples:
  node cli/cli.js add-wallet --xpub "xpub..." --label "Cold Storage"
  node cli/cli.js add-wallet --xpub "xpub..." --label "Hot Wallet" --network testnet
\`\`\`
...
node cli/cli.js balance

# Transaction operations
node cli/cli.js show-transactions
node cli/cli.js show-transactions-global --output csv
node cli/cli.js show-transactions-summary

# System operations
node cli/cli.js show-status
node cli/cli.js backup
node cli/cli.js show-logs
\`\`\`
